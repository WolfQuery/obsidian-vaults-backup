...
const IsDueNextWeekPage = (p) => p.file.tasks
  ...
  .where(
    (t) =>(
      (t.due && (isBetween(-3,-1,DateTime.now().diff(dv.date(t.due),'months').months))) ||
      (t.start && (isBetween(-3,-1,DateTime.now().diff(dv.date(t.start),'months').months))) ||
      (t.scheduled && (isBetween(-3,-1,DateTime.now().diff(dv.date(t.scheduled),'months').months))))
      ).length>0;
...
dv.header(1,DueNextWeekCount+" / 75");
```
...
const IsStartNextWeekPage = (p) => p.file.tasks
  .where((t) => !t.completed)
  .where(t => (t.status != "-"))
  .where((t) => (t.start && (isBetween(-3,-1,DateTime.now().diff(dv.date(t.start),'months').months))))
  .where(
    (t) =>(
	    ((!t.due) ||
	    (t.due && (!isBetween(-3,-1,DateTime.now().diff(dv.date(t.due),'months').months)))) &&
