...
const IsDueNextWeekPage = (p) => p.file.tasks
  ...
  .where(
    (t) =>(
      (t.due && (isBetween(-12,-6,DateTime.now().diff(dv.date(t.due),'months').months))) ||
      (t.start && (isBetween(-12,-6,DateTime.now().diff(dv.date(t.start),'months').months))) ||
      (t.scheduled && (isBetween(-12,-6,DateTime.now().diff(dv.date(t.scheduled),'months').months))))
      ).length>0;
...
dv.header(1,DueNextWeekCount+" / 150");
```

^39b854

### 6-12 Months subjects
...
const IsDueNextWeekPage = (p) => p.file.tasks
  ...
  .where(
    (t) =>(
      (t.due && (isBetween(-12,-6,DateTime.now().diff(dv.date(t.due),'months').months))) ||
      (t.start && (isBetween(-12,-6,DateTime.now().diff(dv.date(t.start),'months').months))) ||
      (t.scheduled && (isBetween(-12,-3,DateTime.now().diff(dv.date(t.scheduled),'months').months))))
      ).length>0;
...
const IsStartNextWeekPage = (p) => p.file.tasks
  .where((t) => !t.completed)
  .where(t => (t.status != "-"))
  .where((t) => (t.start && (isBetween(-12,-6,DateTime.now().diff(dv.date(t.start),'months').months))))
  .where(
    (t) =>(
	    ((!t.due) ||
	    (t.due && (!isBetween(-12,-6,DateTime.now().diff(dv.date(t.due),'months').months)))) &&
