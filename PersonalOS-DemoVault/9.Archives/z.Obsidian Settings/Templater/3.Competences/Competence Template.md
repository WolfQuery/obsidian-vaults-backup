...
if (content === "") {
  tR+= await tp.file.include("[[Competence Metadata 999]]");
}
-%>
# Levels
```dataviewjs
let currentPath = dv.current().file.path
let pages = dv.pages('#Competence-Level')
  .where(p => p.Context && p.Context?.path == currentPath)
  .sort(p => p['Access at level'], 'asc')

dv.list(pages.file.link);
```
# Competence and skill tree
```dataviewjs
let contexts = []; 
let childs = dv.pages('#Competence OR #Skill OR #Tactic');
...
let level = page.Level ? ` (Level: ${page.Level})` : ''; // Add Level property for the current page
